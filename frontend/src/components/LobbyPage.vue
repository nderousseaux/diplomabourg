      ...
      <div id="lobby">
        <div id="joueurs">
          ...
          <div>
            <img
              value="2"
              alt="Attente du joueur"
              title="Attente du joueur"
...
export default {
  ...
  methods: {
    ...
    ready() {
      ...
      api
        .put(
          "/games/" + this.game_id + "/players/" + this.player_id,
          bodyParameters,
          config
        )
        .then((response) => {
  ...
  mounted() {
    ...
    document
      ...
      .addEventListener("click", (event) => {
        ...
        if (erreurForm == false) document.querySelector("form").submit();
      });
    ...
    document.querySelectorAll("#joueurs > div > img").forEach((element) => {
      ...
      if (valeur != undefined) imgAttente.push(element);
    });
...
/* Version tablette */
@media only screen and (hover: none) and (pointer: coarse) and (max-width: 1370px) {
  /* Div principale */
  #app > div > div {
